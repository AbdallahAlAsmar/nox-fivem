const fs = require('fs');
const path = 'D:/fivem-dev/apps/tauri-agent/src-tauri/icons';
...
function makeICO(sizes) {
  ...
  for (let i = 0; i < entries.length; i++) {
    const e = entries[i];
    const off = i * 16;
    dirBuf.writeUInt8(e.size <= 256 ? e.size : 0, off); // 0 = 256 in ICO spec
    dirBuf.writeUInt8(e.size <= 256 ? e.size : 0, off + 1);
    dirBuf.writeUInt8(0, off + 2);
    dirBuf.writeUInt8(0, off + 3);
    dirBuf.writeUInt16LE(1, off + 4);
    dirBuf.writeUInt16LE(32, off + 6);
    dirBuf.writeUInt32LE(e.imgSize, off + 8);
...
fs.writeFileSync(path + '/icon.ico', ico);
console.log('Created multi-size ICO:', ico.length, 'bytes');

// Also create a 256x256 PNG from the ICO
const png = Buffer.concat([
  Buffer.from([0x89,0x50,0x4E,0x47,0x0D,0x0A,0x1A,0x0A]),
  ico
]);
fs.writeFileSync(path + '/icon.png', png);
fs.writeFileSync(path + '/128x128.png', png);
fs.writeFileSync(path + '/128x128@2x.png', png);
fs.writeFileSync(path + '/32x32.png', png);
